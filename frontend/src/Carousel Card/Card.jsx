...
function Card({
  ...
  const props3 = useSpring({
    opacity: 1,
    transform: show ? "scale(1.03) translateY(-10px)" : "scale(1) translateY(0)",
  ...
  return (
    ...
    >
      ...
      <div className="">
        <animated.div
          className="bg-cover bg-center shadow-lg rounded-2xl border-2 border-[#e9dfb2] w-[200px]  sm:w-[20rem] md:w-[25rem] p-4 sm:p-8 md:p-12"
          style={{ backgroundImage: `url(${img})`, ...props3 }}
        >
          <div className={`flex flex-col justify-center text-center text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl pt-4 sm:pt-8 ${color}`}></div>
          <animated.img src={imagen} alt="Your Image" style={show ? props4 : null} />
            ...
            {items.map((item, index) => (
              <Link
                ...
                className={`py-2 px-4 sm:px-6 md:px-8 rounded-xl mt-4 sm:mt-8 text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl tracking-wide transition transform hover:scale-105 duration-300 ${
                  dungeonName === "Aa" ? "bg-yellow-300 text-white" :
                  dungeonName === "Bb" ? "bg-cyan-500 text-white" :
                  dungeonName === "Cc" ? "bg-red-700 text-white" :
                  dungeonName === "Dd" ? "bg-violet-600 text-white" : ""
                }`}
